...
foreach my $line (@GB_file) {
	...
	my $length = ( $to - $from ) + 1;
	my $new_FASTA_header = "$accession\_$length\_$crap_in_the_middle\|$parsed_taxonomy[1]\|$parsed_taxonomy[2]\|$parsed_taxonomy[3]\|$parsed_taxonomy[4]\|$genus\|$species";
	...
	if ($SSU_check == 1 && ($length > $min_length)) {
	    ...
	    #print FILE_CSV1 "$accession,$length,$crap_in_the_middle,$parsed_taxonomy[1],$parsed_taxonomy[2],$parsed_taxonomy[3],$parsed_taxonomy[4],$genus,$species,$extracted_seq\n";
	    #close( FILE_CSV1 );
	    
	    #EXCEL database for ALL phyla
	    unless ( open( ALL_PHYLA_CSV, ">>02_MetaMarks_DB.csv") ) {print "Cannot open file \"02_MetaMarks_DB.csv\" to write to!!\n\n"; exit;}
	    print ALL_PHYLA_CSV "$accession,$length,$crap_in_the_middle,$parsed_taxonomy[1],$parsed_taxonomy[2],$parsed_taxonomy[3],$parsed_taxonomy[4],$genus,$species,$extracted_seq\n";
	    close( ALL_PHYLA_CSV );
	    
	    #FASTA file for each phylum	    
	    my $outfile_fasta = "Phylum_$parsed_taxonomy[1]\.fasta";
	    ...
	    unless ( open( TOO_SHORT, ">>TOO_SHORT.log") ) {print "Cannot open file \"TOO_SHORT.log\" to write to!!\n\n"; exit;}
	    print TOO_SHORT "$accession\n";
	    close( TOO_SHORT );
	} elsif ( $error == 1 ||undef $to || undef $from || ($to - $from) <= 0 ) {
	    unless ( open( TO_CHECK, ">>TO_CHECK.log") ) {print "Cannot open file \"TO_CHECK.log\" to write to!!\n\n"; exit;}
	    print TO_CHECK "$accession\n";
	    close( TO_CHECK );
	}
	...
	if (scalar @positions != 2) { $error = 1;}
	$RNA_flag = 1;		#Flag to indicate seq is RNA
    #Next ELSIF block cheks if former positions belong to 18S, of so keeps them
