...
while (my $line = <IN>){
  ...
  my ($taxa_id,$gene_name) = split /\t/ , $line;
    ...
    if (@gis){						#output to STDOUT about search success
      print "SEARCHRESULTS: @gis\n";
    }else{
      print "NOTFOUND: $search\n";
    }
    EACH_GI:
    foreach my $gi (@gis) {				#for each located ID, record follwing info
      ...
      foreach my $feature_obj ( $seq_obj->get_SeqFeatures ) {	#genbank search and retrieval of feature object
        ...
        if ($feature_obj->has_tag($tag)){
          ...
          $subseq =~ s/(.{60})/$1\n/g;
          print "FOUND: $gene_name gi|$gi|$acc ($range) LEN:${len}bp Ns:$Ns $binomial$sub_species [$lineage]\n";
        	my $print_name = "$acc|$gene_name|$binomial|$sub_species";
	  	$print_name =~ s/\s+/_/g;
	print $out ">$print_name ($range) LEN:${len}bp Ns:$Ns gi|$gi|$acc $gene_name $binomial $sub_species [$lineage]\n$subseq\n";
          last EACH_GI;							
        }else {
          next;
        }
