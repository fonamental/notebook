...
open( my $out, '>', "${input}.SB.fa" ) || die "Can't open file:$!";	#outfile
while (my $line = <IN>){
  	chomp $line;
    ...
    EACH_GI:
    	...
    	foreach my $gi (@gis) {				#for each located ID, record following info
	  		...
	  		my $count = 1;
			while ( my $seq_obj = $db->get_Seq_by_gi( $gi ) ){
    			foreach my $feature_obj ( grep { $_->primary_tag =~ /$featuretype/i } 
    				$seq_obj->get_SeqFeatures ) {
        			my $spliced_feature = $feature_obj->spliced_seq;
        			if( $featuretype =~ /gene|CDS/ ) {
            			$spliced_feature->display_id($feature_obj->has_tag('gene') ? join(',',sort $feature_obj->each_tag_value('gene')) :
                           $featuretype->has_tag('label') ? join(',',$feature_obj->each_tag_value('label')): 
                           $spliced_feature->display_id);
        			} 
        			else {
            			$spliced_feature->display_id(sprintf("%s_%s_%d",
                        $seq_obj->display_id,
                        $feature_obj->primary_tag,
                        $count++));
            		}
				}
       		} 	
