...
#Strings
my ($cln,$org,$mol,$cou,$iso,$env,$date,$seq);
...
foreach my $an (@ans)
{
	...
	#print Dumper $seq_ob;
	($cln,$org,$mol,$cou,$iso,$env,$date,$seq)=(undef,undef,undef,undef,undef,undef,undef,undef);
	...
	FEAT: for $feat($seq_ob->get_SeqFeatures) 
	{
    	...
    	{
        	for my $val($feat->get_tag_values("collection_date")) 
        	{
        		$date.=" $val";
        	}
        }
    ...
    print $outfile "$an;$cln;$org;$mol;$cou;$iso;$env;$date;$seq\n";
    print "$an;$cln;$org;$mol;$cou;$iso;$env;$date\n";
	#exit; #DEBUG
}
close $outfile;
exit;
